...
export type I18nString = keyof I18nFuncs | string;

export function createI18nWrapper(getI18nFn: () => i18n) {
  const I18nMethod = {
    t(key: I18nString, options?: Record<string, unknown>) {
  ...
  return new Proxy(I18nMethod, {
    get(self, key: string | symbol | number) {
      if (key in self) {
        // @ts-expect-error allow
        return self[key];
      }

      return I18nMethod.t.bind(null, key as I18nString);
    },
